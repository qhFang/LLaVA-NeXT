...
class ViTaminDecoder(nn.Module):
    def __init__(
        ...
        self.film_layer_num = film_layer_num
    ...
    def film_forward(self, x):
        x_origin = x
        for i, layer in enumerate(self.blocks):
            if i < self.film_layer_num:
                if self.grad_ckpt:
                    x = checkpoint(layer, x, x_origin)
                else:
                    x = layer(x, x_origin)
            else:
                if self.grad_ckpt:
    ...
    def forward(self, x):
        ...
        x = self.norm_pre(x)
        if self.grad_ckpt:
            if self.film_layer_num == 0:
                x = checkpoint_seq(self.blocks, x)
            else:
                x = self.film_forward(x)
            x = x[:, self.num_query:, :]
            x = self.norm(x)
