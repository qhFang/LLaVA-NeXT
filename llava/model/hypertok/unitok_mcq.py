...
class PlainAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        q, k, v = qkv.reshape(B, N, 3, self.num_heads, self.head_dim).permute(2, 0, 3, 1, 4).unbind(0)    # B， num_heads, N, head_dim

        x = scaled_dot_product_attention(q, k, v)  # B, num_heads, N, head_dim

        if self.in_dim > self.out_dim:
            x = torch.mean(x, dim=1)                                    #B， N， head_dim
            if self.in_dim // self.num_heads != self.out_dim:
                x = nn.functional.adaptive_avg_pool1d(x, self.out_dim)
        else:
            x = x.transpose(1, 2).reshape(B, N, -1)   # B, N, out_dim
        x = self.proj(x)
        return x
...
class VectorQuantizerR(nn.Module):
    def __init__(
        self,
        input_dim,
        vocab_size,
        ...
        self.proj_in = AttnProjection(input_dim, vocab_width, num_heads=num_codebooks)
        ...
        self.proj_out = AttnProjection(vocab_width, input_dim, num_heads=num_codebooks)
    ...
    def forward(self, features):
        ...
        latent_features = torch.zeros_like(features)
        global_vq_loss = 0.
        global_entropy_loss = 0.
        global_vocab_usage = 0.
        for i, codebook in enumerate(self.codebooks):
            latent_feature, vq_loss, entropy_loss, vocab_usage = codebook(features)
            latent_features = latent_features + latent_feature
            features = features - latent_feature
            global_vq_loss += vq_loss
            global_entropy_loss += entropy_loss
            global_vocab_usage += vocab_usage

        latent_features = self.proj_out(latent_features)
...
class VectorQuantizerM(nn.Module):
    ...
    def forward(self, features):
        with torch.amp.autocast(device_type="cuda", enabled=False):
            features = torch.utils.checkpoint.checkpoint(self.proj_in, features, use_reentrant=False)
            ...
            for i, codebook in enumerate(self.codebooks):
                ...
                global_vocab_usage += vocab_usage
            latent_features = torch.cat(latent_features, dim=-1)
            latent_features = torch.utils.checkpoint.checkpoint(self.proj_out, latent_features, use_reentrant=False)
